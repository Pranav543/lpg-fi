 ...
 */
contract LPG is ERC20, ReentrancyGuard {
    error LPG__CollateralAddressesAndPriceFeedAddressesAmountsDontMatch();
    error LPG__TransferFailed();
    error LPG__HealthFactorOk();
    error LPG__TokenNotAllowed(address token);
    error LPG__NeedsMoreThanZero();
    error LPG__BreaksHealthFactor(uint256 healthFactor);
    ...
    modifier isAllowedToken(address token) {
        if (s_priceFeeds[token] == address(0)) {
            revert LPG__TokenNotAllowed(token);
        }
        _;
    }
    ...
    ) ERC20("Synthetic Crude Oil", "LPG") {
        if (collateralAddresses.length != priceFeedAddresses.length) {
            revert LPG__CollateralAddressesAndPriceFeedAddressesAmountsDontMatch();
        }
    ...
    function revertIfHealthFactorIsBroken(address user) internal view {
        ...
        if (healthFactor < MIN_HEALTH_FACTOR) {
            revert LPG__BreaksHealthFactor(healthFactor);
        }
    }
